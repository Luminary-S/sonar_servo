...
*/
void SonarServo::packDataCmd(float Angle, float Speed, float Power,
                             int direction, int mode_num) {
  // uint8_t Data[];
  std::vector<uint8_t> Dat;
  std::cout << "mode_num: " << mode_num << std::endl;
  switch (mode_num) {
    case 0:  // open loop, power should be in -1 ~ 1
    {
      std::cout << "get in mode 1: " << std::endl;
      int16_t power = (int16_t)(Power * 850);
      int cmd_len = 8;
      ...
      Data[3] = 0x02;
      Data[4] = 0xE1;
      Data[5] = (unsigned char)(power & 0xff);
      Data[6] = (unsigned char)((power >> 8) & 0xff);
      for (size_t i = 0; i < cmd_len; i++) {
    ...
    case 1:  //
    {
      std::cout << "get in mode 1: " << std::endl;
      int cmd_len = 10;
      int32_t speed = (int32_t)(angles::to_degrees(Speed) * 100);
      uint8_t Data[cmd_len];
      Data[0] = 0x3E;
      Data[1] = 0xA2;
      Data[2] = 0X00 + MOTOR_ID_;
      Data[3] = 0x04;
      Data[4] = 0xE5;
      Data[5] = (unsigned char)(speed & 0xff);
      Data[6] = (unsigned char)((speed >> 8) & 0xff);
      Data[7] = (unsigned char)((speed >> 16) & 0xff);
      Data[8] = (unsigned char)((speed >> 24) & 0xff);
      for (size_t i = 0; i < cmd_len; i++) {
        Dat.push_back(Data[i]);
      }
      getChecksum(Dat, 0, 4);
    ...
    case 2: {
      ...
      int cmd_len = 14;
      int64_t angle = (int64_t)(angles::to_degrees(Angle) * 100);
      uint8_t Data[cmd_len];
      Data[0] = 0x3E;
      Data[1] = 0xA3;
      Data[2] = 0X00 + MOTOR_ID_;
    ...
    {
      int cmd_len = 10;
      int32_t angle = (int32_t)(angles::to_degrees(Angle) * 100);
      uint8_t Data[cmd_len];
      Data[0] = 0x3E;
      Data[1] = 0xA5;
      Data[2] = 0X00 + MOTOR_ID_;
...
/*
get target of the motor
*/
float SonarServo::set_target() {
  // ROS_INFO("set target part");
  direction = 0;
  delta_yaw = angles::shortest_angular_distance(init_yaw, now_yaw);

  angle = angles::normalize_angle_positive(
      init_angle + delta_yaw);  // - adjust_angle_ * DEGREE_TO_RADIAN;

  delta_init = angles::shortest_angular_distance(
  ...
  if (abs(delta_init) < angles::from_degrees(angle_threshold_)) {
    angle = now_angle;
  }
  // target = (int)(angles::to_degrees(angle) * 100);  // angle/2/3.14 * 2048;
  // //
  return angle;
}

float SonarServo::get_angle_interval(float des, float now) {
  return angles::shortest_angular_distance(angles::normalize_angle(now),
                                           angles::normalize_angle(des));
}
/*
speed control using cmd mode 1
*/
float SonarServo::speed_controller(float target, float position,
                                   float interval) {
...
/*
speed control using cmd mode 4
*/
// float SonarServo::position_controller(float target, int direction) {
//   if (abs(delta_init) < angles::from_degrees(5.0)) {
//     std::cout << "== less than threshold ==" << std::endl;
//     stop();
//   } else {
//     // packDataCmd(target, 0, 0, direction, 4);
//   }
//   return;
// }

void SonarServo::sendCmd(float target, float speed, int StopBit) {
  // ROS_INFO(" send cmd part ");
  if (StopBit) {
    // std::cout << "== less than threshold ==" << std::endl;
    stop();
  } else {
    // packDataCmd(target, 0, 0, direction, 4);
    packDataCmd(target, speed, 0, direction, 1);
  }
  return;
}
...
SonarServo::SonarServo(ros::NodeHandle &nh, ros::NodeHandle &nh_private) {
  ...
  if (sonar_ser_.isOpen()) {
    ...
    while (ros::ok()) {
      ...
      if (!get_init_data(num)) {
        continue;
      }

      // now_angle = getEncoderData();
      float target_des = set_target();
      float speed_des =
          speed_controller(target_des, now_angle, 1.0 / pub_rate_);
      // float target = position_controller(target_des, direction);
      ...
      // }
      stopbit = 0;
      sendCmd(target_des, speed_des, stopbit);
      publish();
      // if (num > 50) {
      //   break;
      // }
      // ROS_INFO("---------------end loop times: %d---------------------",
      // num);
      r.sleep();
    }
  }
...
bool SonarServo::get_init_data(int num) {
  // int temp = 0;
  // int flg = 1;
  float temp_angle;
  ...
  if ((num - temp_num_) < 10) {
    ...
    ROS_INFO("------out 2--%d---%d---", temp_num_, num);
    return false;
  } else if ((num - temp_num_) == 10) {
